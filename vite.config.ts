  ...
  vite: {
    ...
    plugins: [
      VitePWA({
        ...
        manifest: false,
        workbox: {
          // vite-plugin-pwa defaults this to "index.html". Disable that
          // generated NavigationRoute explicitly; document caching is handled
          // by the guarded NetworkFirst runtime route below.
          navigateFallback: null,
          // Keep only static, versioned resources in the precache. HTML is
          ...
          runtimeCaching: [
            ...
            {
              // HTML documents are always network-first. API/server-function,
              // auth callback, worker and static-asset URLs are excluded even
              // when opened directly in a browser tab, so they can never be
              // mistaken for cacheable application HTML.
              urlPattern: ({ url, request, sameOrigin }) => {
                ...
                const isApplicationEndpoint =
                  /^\/(?:api|auth|~oauth|_serverFn|_worker)(?:\/|$)/.test(path);
                const isStaticPath =
                  /^\/(?:assets|branding|companions|geography|__l5e)(?:\/|$)/.test(path) ||
                  /\.(?:css|js|mjs|map|json|png|jpe?g|gif|svg|webp|avif|ico|woff2?|ttf|otf|webmanifest|xml|txt)$/i.test(
                    path,
                  );

                return !isApplicationEndpoint && !isStaticPath;
              },
