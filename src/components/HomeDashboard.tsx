  ...
  return (
    <section className="px-4 py-6 pb-[calc(var(--mobile-content-bottom)+1rem)] sm:px-6 lg:px-8 lg:pb-10 space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_300px]">
        ...
        {/* Explorer command card */}
        <div className="flex flex-col items-center rounded-[2rem] border border-white/[0.1] bg-gradient-to-b from-[#1E1B4B]/45 to-[#0B1220]/70 p-5 text-center backdrop-blur-2xl">
          <ExplorerChamber config={progress.avatar} titleId={progress.equippedTitle} size={128} rankGlowColor={rank.glowColor} rankName={rank.name} />
          <div className="mt-1 flex items-center gap-1.5 text-sm font-bold text-[#FCD34D]">
            <Sparkles className="h-4 w-4" /> {tokens.toLocaleString()} Stardust
          </div>
          <Link
            to="/market"
            className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#FBBF24]/30 bg-[#FBBF24]/10 px-4 py-2.5 text-xs font-bold text-[#FCD34D] transition-colors hover:bg-[#FBBF24]/20"
          >
            <Store className="h-4 w-4" /> Open Explorer Hub
          </Link>
        </div>
      </section>
