...
const modules: MissionDefinition[] = [
  ...
  { number: "03", kindLabel: "Modul", title: "Contoh & Latihan", description: "Pelajari contoh karangan UASA lengkap mengikut tema popular.", icon: Brain, color: "#C084FC" },
];
...
export function BMForm2KaranganPanjangContent({ initialSectionId, onBack }: { initialSectionId?: string; onBack?: () => void } = {}) {
  ...
  if (activeModule === null) return <ExamSkillLanding title="Karangan Panjang" subtitle="Bahagian B: Kuasai karangan melebihi 180 patah perkataan langkah demi langkah." missions={modules} onSelect={setActiveModule} />;
  const module = modules[activeModule];
  return <MissionPageShell mission={module} onBack={() => onBack ? onBack() : setActiveModule(null)}>
    {activeModule === 0 && <>
      <section className="rounded-2xl border border-indigo-300/20 bg-indigo-300/[0.05] p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-indigo-300">Mission Brief</p><p className="mt-2 text-sm leading-7 text-white/70">Kuasai format, kembangkan sekurang-kurangnya 3 isi menggunakan formula IMBAK dan hasilkan karangan melebihi 180 patah perkataan.</p></section>
      <MissionSection title="Apa Itu Karangan Panjang" color={module.color}><div className="grid gap-3 sm:grid-cols-3">{[["Tujuan", "Mengembangkan sesuatu tema menjadi karangan yang lengkap dan matang."], ["Panjang", "Melebihi 180 patah perkataan"], ["Format", "Bahagian B · Kertas 2"]].map(([label, text]) => <div key={label} className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-4"><p className="text-[10px] font-black uppercase tracking-widest text-amber-300">{label}</p><p className="mt-2 text-sm leading-6 text-white/70">{text}</p></div>)}</div></MissionSection>
