...
function PantunBudiContent() {
  ...
  return (
    <div className="divide-y divide-white/[0.06]">
      <StudySection>
        <div className="rounded-2xl border border-sky-300/15 bg-sky-300/[0.06] p-5 sm:p-6">
          <SectionTitle icon={<Target className="h-5 w-5" />} eyebrow="Misi Pembelajaran" title="Budi dikenang, ilmu dikuasai" color="#60A5FA" />
          <p className="mb-4 max-w-2xl text-sm leading-7 text-white/65">{note.mission}</p>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{["Budi bahasa", "Mengenang jasa", "Tema dan persoalan", "Nilai murni", "Pengajaran", "Persediaan UASA"].map((item) => <div key={item} className="flex items-center gap-2 rounded-xl bg-black/15 px-3 py-2.5 text-xs font-semibold text-white/75"><Check className="h-3.5 w-3.5 text-emerald-400" />{item}</div>)}</div>
        </div>
      </StudySection>
      ...
      <StudySection>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {note.illustrations.map((explanation, index) => <article key={explanation} className="rounded-2xl border border-white/[0.08] bg-white/[0.035] p-4 transition-colors hover:bg-white/[0.055]"><div className="mb-3 flex items-center justify-between"><Pill>Rangkap {index + 1}</Pill><Quote className="h-4 w-4 text-white/20" /></div><p className="text-sm leading-6 text-white/75">{explanation}</p></article>)}
        </div>
      </StudySection>

      <StudySection>
        <SectionTitle icon={<Sparkles className="h-5 w-5" />} eyebrow="Idea Utama" title="Tema" color="#F472B6" />
        <div className="rounded-2xl border border-pink-300/20 bg-pink-300/[0.08] p-5 sm:p-6"><Pill color="#F472B6">Tema Utama</Pill><h3 className="mt-4 font-display text-lg font-bold text-white">{note.theme.title}</h3><p className="mt-3 text-sm leading-6 text-white/65">{note.theme.example}</p><div className="mt-3"><Pill color="#F472B6">{note.theme.reference}</Pill></div></div>
      </StudySection>

      <StudySection>
        <SectionTitle icon={<ListChecks className="h-5 w-5" />} eyebrow="Kupas Karya" title="Persoalan" color="#60A5FA" />
        <div className="grid gap-3 sm:grid-cols-2">{note.issues.map((item, index) => <article key={item.title} className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4"><div className="mb-3 flex items-start gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-sky-400/10 text-xs font-black text-sky-300">{index + 1}</span><h3 className="text-sm font-bold leading-6 text-white">{item.title}</h3></div><p className="text-xs leading-5 text-white/60">{item.example}</p><div className="mt-3"><Pill color="#60A5FA">{item.reference}</Pill></div></article>)}</div>
      </StudySection>
      ...
      <StudySection>
        ...
        <div className="grid gap-3 sm:grid-cols-2">{note.language.map((item) => <article key={item.title} className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4"><h3 className="mb-3 font-bold text-white">{item.title}</h3><div className="space-y-2">{item.examples.map((example) => <p key={example} className="rounded-xl bg-amber-300/[0.07] px-3 py-2.5 text-xs italic leading-5 text-amber-100/80">“{example}”</p>)}</div></article>)}</div>
      </StudySection>

      <StudySection>
        <SectionTitle icon={<Heart className="h-5 w-5" />} eyebrow="Bina Peribadi" title="Nilai Murni" color="#FB7185" />
        <div className="grid gap-3 sm:grid-cols-2">{note.values.map((item, index) => { const color = VALUE_COLORS[index % VALUE_COLORS.length]; return <article key={item.title} className="rounded-2xl border bg-white/[0.03] p-4" style={{ borderColor: `${color}25` }}><div className="mb-3 flex items-center gap-2"><Heart className="h-4 w-4" style={{ color }} /><h3 className="font-bold text-white">{item.title}</h3></div><p className="text-sm leading-6 text-white/65">{item.example}</p><div className="mt-3"><Pill color={color}>{item.reference}</Pill></div></article>; })}</div>
      </StudySection>

      <StudySection>
        <SectionTitle icon={<CheckCircle2 className="h-5 w-5" />} eyebrow="Amalkan" title="Pengajaran" color="#34D399" />
        <div className="grid gap-3 sm:grid-cols-2">{note.values.map((item) => <article key={item.title} className="flex gap-3 rounded-2xl border border-emerald-300/10 bg-emerald-300/[0.05] p-4"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" /><div><h3 className="text-sm font-bold text-white">{item.title}</h3><p className="mt-1 text-xs leading-5 text-white/60">{item.lesson}</p></div></article>)}</div>
      </StudySection>
      ...
      <StudySection>
        <div className="overflow-hidden rounded-2xl border border-purple-300/20 bg-[#151327]">
          <div className="border-b border-white/[0.07] p-5 sm:p-6"><SectionTitle icon={<Lightbulb className="h-5 w-5" />} eyebrow="Ulang Kaji Bersama" title="Cikgu AcadeMY" color="#C084FC" /><p className="text-sm leading-7 text-white/65">Gunakan kata kunci dan struktur jawapan yang tepat untuk mengulang kaji dengan lebih teratur.</p></div>
          <div className="grid gap-px bg-white/[0.06] sm:grid-cols-2 lg:grid-cols-3">
            <TeacherTip icon={<Star className="h-4 w-4" />} title="Apa yang penting?" color="#FBBF24">Asingkan tema, persoalan, nilai murni dan pengajaran dalam empat kumpulan nota.</TeacherTip>
            <TeacherTip icon={<Target className="h-4 w-4" />} title="Fokus UASA" color="#34D399">Baca kata tugas soalan dan pilih kategori jawapan sebelum menulis.</TeacherTip>
            <TeacherTip icon={<BookOpen className="h-4 w-4" />} title="Tip Menghafal" color="#60A5FA">Gunakan kad imbas: istilah di hadapan, penerangan dan contoh di belakang.</TeacherTip>
            <TeacherTip icon={<ShieldAlert className="h-4 w-4" />} title="Kesalahan Lazim" color="#FB7185">Jangan campurkan label nilai murni dengan ayat pengajaran.</TeacherTip>
            <TeacherTip icon={<CheckCircle2 className="h-4 w-4" />} title="Cara Menjawab" color="#C084FC">Mulakan dengan isi yang diminta, kemudian sertakan contoh atau rujukan rangkap yang sepadan.</TeacherTip>
          </div>
        </div>
      </StudySection>
    </div>
