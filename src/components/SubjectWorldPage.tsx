...
const WORLDS: Record<string, WorldConfig> = {
  math: {
    ...
    worldSubtitle:
      "An infinite universe of numbers, formulas, and elegant solutions waiting to be discovered",
    ...
    chapterLabel: "Station",
    sectionLabel: "Galaxy Stations",
  ...
  science: {
    ...
    worldSubtitle:
      "Where curiosity meets experiment and every observation reveals a new truth about the universe",
  ...
  english: {
    ...
    worldSubtitle:
      "A boundless cosmos of stories, words, grammar, and the infinite power of human expression",
    ...
    chapterLabel: "Zone",
    sectionLabel: "Language Zones",
    ctaLabel: "Enter Zone",
    backLabel: "Exit Universe",
    ambientSymbols: ["❝", "A", "Z", "...", "→", "!", "?", "✦"],
    identityBar:
      "❝  Vocabulary  ·  Grammar  ·  Reading  ·  Writing  ·  Expression  ·  Communication  ❞",
  ...
  geography: {
    ...
    worldSubtitle:
      "Chart unknown territories, decode Earth's systems, and conquer every terrain from pole to pole",
  ...
  sejarah: {
    ...
    worldSubtitle:
      "Travel through centuries, witness empires rise and fall, and uncover Malaysia's greatest legacies",
    ...
    chapterLabel: "Era",
    sectionLabel: "Historical Eras",
    ctaLabel: "Travel Through Time",
    backLabel: "Return to Present",
    ambientSymbols: ["1957", "BCE", "AD", "⚔", "📜", "Abad", "1400", "🏛"],
    identityBar:
      "PRASEJARAH   ·   TAMADUN AWAL   ·   YUNANI & ROM   ·   TAMADUN ISLAM   ·   MALAYSIA MERDEKA",
  ...
  bm: {
    ...
    worldSubtitle:
      "Jelajah dunia sastera, kuasai bahasa kebangsaan, dan warisi budaya bangsa yang agung dan mulia",
    ...
    chapterLabel: "Daerah",
    sectionLabel: "Daerah Pembelajaran",
    ctaLabel: "Mula Perjalanan",
    backLabel: "Kembali",
    ambientSymbols: ["ب", "Kata", "Prosa", "Sajak", "Cerpen", "Tata", "Puisi", "BM"],
    identityBar:
      "TATABAHASA   ·   KOSA KATA   ·   PENULISAN   ·   SASTERA   ·   PUISI   ·   CERPEN   ·   PERIBAHASA",
...
const MATH_F1_BACKGROUNDS = buildChapterBackgroundMap(
  mathF1BgModules,
  /^math_f1_chapter(\d{1,2})/i,
);
const MATH_F2_BACKGROUNDS = buildChapterBackgroundMap(
  mathF2BgModules,
  /^math_f2_chapter(\d{1,2})/i,
);
const MATH_F3_BACKGROUNDS = buildChapterBackgroundMap(
  mathF3BgModules,
  /^math_f3_chapter(\d{1,2})/i,
);
...
const SEJARAH_F1_BACKGROUNDS = buildChapterBackgroundMap(
  sejarahF1BgModules,
  /^sejarah_f1_chapter(\d{1,2})/i,
);
const SEJARAH_F2_BACKGROUNDS = buildChapterBackgroundMap(
  sejarahF2BgModules,
  /^sejarah_f2_chapter(\d{1,2})/i,
);
const SEJARAH_F3_BACKGROUNDS = buildChapterBackgroundMap(
  sejarahF3BgModules,
  /^sejarah_f3_chapter(\d{1,2})/i,
);
...
const SLOTS = [
  ...
  { top: "44%", right: "1.5%", cls: "particle-float-1", op: 0.1, fs: "0.8rem" },
        ...
        {config.badges.map((badge, i) => (
          ...
          >
            <div
              ...
              style={{
                filter: unlocked[i] ? "none" : "grayscale(1)",
                opacity: unlocked[i] ? 1 : 0.35,
              }}
            >
              {badge.emoji}
            </div>
            <p
              ...
              style={{ color: unlocked[i] ? config.color : "rgba(255,255,255,0.3)" }}
            >
              {badge.name}
            </p>
...
type MapProps = {
  ...
  notesProgress?: NotesProgressMap;
  onSelect: (key: string) => void;
};

function displayedChapterProgress(
  progress: ProgressMap,
  notesProgress: NotesProgressMap | undefined,
  resourceType: ResourceType,
  subjectId: string,
  chapterKey: string,
) {
  return selectNotesAwareProgress(
    resourceType,
    notesProgress?.[chapterKey],
    progress.chapterActivity[chapterActivityKey(subjectId, chapterKey)],
  );
}
    ...
    <div
      ...
      style={{
        ...
        boxShadow: isComplete
          ...
          : isStarted
            ? `0 0 ${pct < 25 ? 6 : pct < 50 ? 10 : pct < 75 ? 14 : 18}px ${config.color}${pct < 25 ? "25" : pct < 50 ? "40" : pct < 75 ? "60" : "85"}`
            : "none",
      }}
    >
      {/* Progress ring */}
      ...
      {!chapter.available ? (
        ...
        <span
          className="relative z-10 font-display text-sm font-black"
...
function LocationCard({
  ...
  const pct = displayedChapterProgress(
    progress,
    notesProgress,
    resourceType,
    subjectId,
  ...
  const scienceBackgrounds =
    form === "Form 1"
      ? SCIENCE_F1_BACKGROUNDS
      : form === "Form 2"
        ? SCIENCE_F2_BACKGROUNDS
        : form === "Form 3"
          ? SCIENCE_F3_BACKGROUNDS
          : undefined;
  const bgImage =
    subjectId === "geography" && form === "Form 1"
      ...
      : subjectId === "geography" && form === "Form 2"
        ...
        : subjectId === "geography" && form === "Form 3"
          ? GEOGRAPHY_F3_BACKGROUNDS[chapter.key]
          : subjectId === "math" && form === "Form 1"
            ? MATH_F1_BACKGROUNDS[chapter.key]
            : subjectId === "math" && form === "Form 2"
              ? MATH_F2_BACKGROUNDS[chapter.key]
              : subjectId === "math" && form === "Form 3"
                ? MATH_F3_BACKGROUNDS[chapter.key]
                : subjectId === "science"
                  ? scienceBackgrounds?.[chapter.key]
                  : undefined;
  const bgImagePosition =
    subjectId === "science" && form === "Form 1"
      ? (SCIENCE_F1_BG_POSITION[chapter.key] ?? "center")
      : subjectId === "science" && form === "Form 2"
        ? (SCIENCE_F2_BG_POSITION[chapter.key] ?? "center")
        : subjectId === "science" && form === "Form 3"
          ? (SCIENCE_F3_BG_POSITION[chapter.key] ?? "center")
          : "center";
...
function VerticalPathMap({
  ...
  return (
    <div className="relative py-2">
      {/* Desktop centre path */}
      <div
        className="absolute top-7 bottom-7 hidden sm:block"
        style={{ left: "50%", width: 2, transform: "translateX(-50%)", background: pathLine }}
      />
      {/* Mobile left path */}
      <div
        className="absolute top-7 bottom-7 sm:hidden"
...
function SejarahTimelineMap({
  ...
  const eraBackgrounds =
    form === "Form 1"
      ? SEJARAH_F1_BACKGROUNDS
      : form === "Form 2"
        ? SEJARAH_F2_BACKGROUNDS
        : form === "Form 3"
          ? SEJARAH_F3_BACKGROUNDS
          : undefined;
  ...
  return (
    <div className="-mx-2 overflow-x-auto pb-6">
      <div
        ...
        style={{ width: totalW, minWidth: "100%", paddingTop: 8 }}
      >
        {/* Era epoch labels */}
        <div className="flex w-full">
          {chapters.map((c, i) => (
            <div key={`era-${c.key}`} className="flex justify-center" style={{ width: NODE_W }}>
              <p
                className="mb-1 text-[7.5px] font-black uppercase tracking-widest"
        ...
        <div className="relative flex w-full items-center" style={{ height: 60 }}>
          ...
          {chapters.map((c, i) => {
            ...
            const thisPct = displayedChapterProgress(
              progress,
              notesProgress,
              resourceType,
              subjectId,
            ...
            const nextPct = displayedChapterProgress(
              progress,
              notesProgress,
              resourceType,
              subjectId,
            ...
            return (
              <div
                key={`seg-${c.key}`}
                className="absolute top-1/2 -translate-y-1/2 h-0.5 transition-all"
                style={{
                  left: (i + 0.5) * NODE_W,
                  width: NODE_W,
        ...
        <div className="flex w-full">
          {chapters.map((c, i) => {
            ...
            return (
              <div
                key={`card-${c.key}`}
                className="animate-slide-up px-1.5 pt-3"
              ...
              >
                <button
                  ...
                  className={[
                    ...
                    canOpen
                      ? "cursor-pointer hover:border-white/[0.14]"
                      : "opacity-38 cursor-not-allowed",
                  ].join(" ")}
                  style={{
                    background: `linear-gradient(180deg, ${config.from}0d, rgba(0,0,0,0.5))`,
                    border: "1px solid rgba(255,255,255,0.06)",
                  }}
                  onMouseEnter={(e) => {
                    if (canOpen)
                      (e.currentTarget as HTMLElement).style.borderColor = `${config.color}35`;
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <div className="relative z-10">
                    {c.categoryLabel && canOpen && (
                      <p
                        data-card-eyebrow
                        className="mb-0.5 text-[7.5px] font-black uppercase tracking-widest"
      ...
      <div className="p-4">
        ...
        {location && chapter.available && (
          <p
            ...
            style={{ color: config.color, opacity: 0.65 }}
          >
            {location}
          </p>
        )}
        ...
        {chapter.available && (
          <div className="mt-3">
            <div className="mb-1 flex justify-between text-[9px] font-bold uppercase tracking-wide">
              ...
              <span style={{ color: pct > 0 ? config.color : "rgba(255,255,255,0.2)" }}>
                {pct}%
              </span>
            </div>
...
export function SubjectWorldPage({
  ...
  const completedCount = chapters.filter(
    (c) => displayedChapterProgress(progress, notesProgress, resourceType, subjectId, c.key) >= 100,
  ).length;
    ...
    availableChapters.length > 0
      ? Math.round(
          availableChapters.reduce(
            (sum, c) =>
              sum +
              displayedChapterProgress(progress, notesProgress, resourceType, subjectId, c.key),
            0,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle, rgba(255,255,255,0.75) 1px, transparent 1px), radial-gradient(circle, rgba(255,255,255,0.3) 1px, transparent 1px)",
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 pt-6 sm:px-8">
        ...
        ══════════════════════════════════════════════════════ */}
        <div
          className="relative mb-6 overflow-hidden rounded-[2rem] border border-white/[0.08]"
          style={{
        ...
        >
          ...
          <div
            ...
            style={{
              background: `radial-gradient(circle at 80% 20%, ${config.from}20, transparent 55%)`,
            }}
          />
          {/* Star micro-dots */}
          <div
          ...
          {/* Identity ticker bar */}
          <div
            className="border-t border-white/[0.05] px-6 py-2.5"
            style={{ background: `${config.from}09` }}
          >
            <p
              className="truncate text-center text-[10px] font-bold tracking-[0.18em]"
              style={{ color: config.color, opacity: 0.55 }}
            >
              {config.identityBar}
            </p>
        ...
        <div className="mb-6 flex items-center gap-4">
          <div
            ...
            style={{ background: `linear-gradient(90deg, ${config.from}45, transparent)` }}
          />
          <p
          ...
          <div
            className="h-px flex-1"
            style={{ background: `linear-gradient(270deg, ${config.from}45, transparent)` }}
          />
        </div>
        ...
        {chapters.length === 0 ? (
          ...
          <div
            aria-busy={resourceType === "notes" && notesProgressLoading}
            className={
              resourceType === "notes" && notesProgressLoading
                ? "opacity-55 transition-opacity motion-reduce:transition-none"
                : "transition-opacity motion-reduce:transition-none"
            }
          >
            {resourceType === "notes" && notesProgressLoading && (
              <span className="sr-only">Loading saved Notes progress</span>
            )}
            <ChapterPathMap
              chapters={chapters}
              config={config}
