...
import { DecisionTree } from "./blocks/CentralTendencyDiagrams";
...
/** Form 2 Mathematics, Chapter 12 (Measures of Central Tendencies) interactive notes. */
export function MathF2Chapter12NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C12Content; bm: MathF2C12Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c12-${s.num}`, weight: 3 })),
    { id: "math-f2-c12-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c12-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual && (
              <DecisionTree
                categoryLabel={sub.visual.categoryLabel}
                useModeLabel={sub.visual.useModeLabel}
                extremeLabel={sub.visual.extremeLabel}
                useMedianLabel={sub.visual.useMedianLabel}
                useMeanLabel={sub.visual.useMeanLabel}
                yesLabel={sub.visual.yesLabel}
                noLabel={sub.visual.noLabel}
      ...
      <div data-notes-section-id="math-f2-c12-chapter-end" className="mt-14 space-y-10">
        <div>
