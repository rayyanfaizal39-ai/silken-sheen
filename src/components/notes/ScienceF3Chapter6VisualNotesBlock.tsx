...
const copy = {
  bm: {
    ...
    renewableDesc: "Sumber yang boleh digantikan secara berterusan.",
  ...
  en: {
    ...
    hide: "Hide answer",
    renewable: "Renewable",
    renewableDesc: "Sources that can be replaced continually.",
...
function SectionTitle({ number, title, subtitle }: { number: string; title?: string; subtitle?: string }) {
  return <div className="space-y-2"><div className="inline-flex rounded-full border border-cyan-300/25 bg-cyan-300/10 px-3 py-1 text-xs font-bold tracking-[.16em] text-cyan-200">{number}</div>{title && <h2 className="font-display text-2xl font-black leading-tight text-white sm:text-3xl">{title}</h2>}{subtitle && <p className="max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">{subtitle}</p>}</div>;
}
