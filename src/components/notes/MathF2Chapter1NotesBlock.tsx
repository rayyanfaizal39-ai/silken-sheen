...
import {
  GrowingDotPatternDiagram,
  SequenceGapCheckerDiagram,
  NthTermMachineDiagram,
  PascalTriangleDiagram,
} from "./blocks/PatternDiagrams";
...
/** Form 2 Mathematics, Chapter 1 (Patterns and Sequences) interactive notes. */
export function MathF2Chapter1NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C1Content; bm: MathF2C1Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c1-${s.num}`, weight: 3 })),
    { id: "math-f2-c1-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c1-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.introVisual?.kind === "dotPattern" && (
              <GrowingDotPatternDiagram
                counts={sub.introVisual.counts}
                caption={sub.introVisual.caption}
              />
            )}
            {sub.introVisual?.kind === "gapCheck" && (
              <SequenceGapCheckerDiagram
                good={sub.introVisual.good}
                bad={sub.introVisual.bad}
                goodLabel={sub.introVisual.goodLabel}
                badLabel={sub.introVisual.badLabel}
              />
            )}
            {sub.introVisual?.kind === "ntermMachine" && (
              <NthTermMachineDiagram
                inputLabel={sub.introVisual.inputLabel}
                formula={sub.introVisual.formula}
                outputLabel={sub.introVisual.outputLabel}
                caption={sub.introVisual.caption}
              />
            )}
          </LessonIntro>
          ...
          {sub.afterMistakeVisual?.kind === "pascalTriangle" && (
            <PascalTriangleDiagram
              rows={sub.afterMistakeVisual.rows}
              caption={sub.afterMistakeVisual.caption}
            />
          )}
      ...
      <div data-notes-section-id="math-f2-c1-chapter-end" className="mt-14 space-y-10">
        <div>
