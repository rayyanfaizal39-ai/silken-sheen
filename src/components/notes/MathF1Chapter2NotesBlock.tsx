...
import {
  ArrayGridDiagram,
  MultiplesTimelineDiagram,
  VennHCFDiagram,
} from "./blocks/FactorMultipleDiagrams";
...
/**
 * Form 1 Mathematics, Chapter 2 (Factors and Multiples) interactive notes.
 * Follows the same composition as MathF1Chapter9NotesBlock, but replaces
 * FormulaCard/StepsCard with LadderCard/LadderWorkedExample wrapping the
 * shared DivisionLadder component (same hcfLadderRows/lcmLadderRows
 * algorithm already live in Chapter 4) for both the "how the method works"
 * demo and the worked example — the ladder reveal IS the worked example,
 * there's no separate step-by-step calc/why list here.
 */
export function MathF1Chapter2NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF1C2Content; bm: MathF1C2Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f1-c2-${s.num}`, weight: 3 })),
    { id: "math-f1-c2-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f1-c2-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.arrayGrid && (
              <ArrayGridDiagram
                n={sub.arrayGrid.n}
                rows={sub.arrayGrid.rows}
                cols={sub.arrayGrid.cols}
                caption={sub.arrayGrid.caption}
              />
            )}
            {sub.vennCaption && <VennHCFDiagram caption={sub.vennCaption} />}
            {sub.multiplesCaption && <MultiplesTimelineDiagram caption={sub.multiplesCaption} />}
          </LessonIntro>

          <LadderCard eyebrow={sub.ladderDemo.eyebrow}>
            <DivisionLadder
              lang={lang}
              mode={sub.ladderDemo.mode}
          ...
          <LadderWorkedExample lang={lang} question={sub.worked.question}>
            <DivisionLadder
              lang={lang}
              mode={sub.worked.mode}
              numbers={sub.worked.numbers}
              reveal="answer"
            />
          </LadderWorkedExample>
      ...
      <div data-notes-section-id="math-f1-c2-chapter-end" className="mt-14 space-y-10">
        <div>
