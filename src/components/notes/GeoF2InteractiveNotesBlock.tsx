...
export function GeoF2InteractiveNotesBlock({
  id,
  content,
  storageKey,
  isRead,
  ...
  id?: string;
  content: GeoF2InteractiveContent;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
}) {
    ...
    <section id={id} className="mt-8 flex min-w-0 flex-col gap-9 animate-fade-up">
      ...
      {content.sections.map((section) => (
        <div key={section.number} className="flex min-w-0 flex-col gap-5">
          ...
          {section.gridReference && (
            <div>
              <h3 className="font-display mb-2 text-base font-bold text-foreground">{section.gridReference.title}</h3>
              <p className="mb-2 text-[13px] leading-relaxed text-muted-foreground">{section.gridReference.instruction}</p>
              <GridReferenceReader block={section.gridReference} onComplete={() => awardOnce(`grid-${section.number}`, 20)} />
            </div>
          )}
          <div>
      ...
      <div>
        ...
        <SelfReflectionChecklist items={content.reflectionItems} storageKey={storageKey ? `${storageKey}:geo-f2-c${content.chapter}-reflection` : undefined} onAllComplete={() => awardOnce("reflection", 10)} />
      </div>
      <div>
