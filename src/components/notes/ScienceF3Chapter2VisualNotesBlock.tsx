...
const copy = {
  bm: {
    ...
    title: "Respirasi",
    subtitle: "Sistem respirasi, pergerakan dan pertukaran gas dalam badan manusia, kesihatan sistem respirasi manusia, adaptasi dalam sistem respirasi dan pertukaran gas dalam tumbuhan.",
    roadmap: [
      ["2.1 Sistem Respirasi", "Bagaimana udara bergerak dan perubahan isi padu serta tekanan menghasilkan pernafasan."],
      ["2.2 Pergerakan dan Pertukaran Gas di dalam Badan Manusia", "Bagaimana resapan, alveolus dan hemoglobin menghantar oksigen kepada sel."],
      ["2.3 Kesihatan Sistem Respirasi Manusia", "Bagaimana bahan pencemar dan asap rokok merosakkan laluan udara dan alveolus."],
      ["2.4 Adaptasi dalam Sistem Respirasi", "Bagaimana katak, ikan dan serangga menyesuaikan permukaan pertukaran gas."],
  ...
  en: {
    ...
    title: "Respiration",
    subtitle: "Human respiratory system, movement and exchange of gases in the human body, health of human respiratory system, adaptations in respiratory systems and gaseous exchange in plants.",
    roadmap: [
      ["2.1 Human Respiratory System", "How air moves and changes in volume and pressure produce breathing."],
      ["2.2 Movement and Exchange of Gases in the Human Body", "How diffusion, alveoli and haemoglobin deliver oxygen to cells."],
      ["2.3 Health of Human Respiratory System", "How pollutants and cigarette smoke damage airways and alveoli."],
      ["2.4 Adaptations in Respiratory Systems", "How frogs, fish and insects adapt their gaseous-exchange surfaces."],
...
function PollutantExplorer({ lang }: { lang: Lang }) {
  const pollutants = lang === "bm" ? [
    ...
    ["Karbon monoksida", "Tidak berwarna dan tidak berbau", "CO + hemoglobin → karboksihemoglobin stabil; pengangkutan O₂ dan tenaga sel berkurang."],
...
export function ScienceF3Chapter2VisualNotesBlock({ id, content, lang, isRead, onMarkRead }: { id?: string; content: ScienceF3InteractiveContent; lang: Lang; storageKey?: string; isRead?: boolean; onMarkRead?: () => void }) {
  ...
  return <section id={id} data-lang={lang} className="relative mt-8 min-w-0 overflow-hidden rounded-[2rem] border border-sky-300/15 bg-[#0b1724] px-4 py-6 text-slate-100 shadow-2xl sm:px-7 lg:px-9"><div className="pointer-events-none absolute inset-x-0 top-0 h-96 bg-[radial-gradient(circle_at_18%_14%,rgba(56,189,248,.18),transparent_38%),radial-gradient(circle_at_82%_18%,rgba(16,185,129,.14),transparent_34%)]"/><div className="relative mx-auto flex max-w-6xl flex-col gap-14">
    <header className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-gradient-to-br from-sky-400/15 via-slate-950/40 to-emerald-400/15 p-5 sm:p-8"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.2em] text-sky-200"><Wind className="h-4 w-4"/>{t.eyebrow}</div><h1 className="mt-3 max-w-4xl font-display text-4xl font-black leading-[1.02] text-white sm:text-5xl">{t.title}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-slate-300">{t.subtitle}</p><div className="mt-7 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{t.roadmap.map((item, index) => <div key={item[0]} className="relative min-h-40 rounded-2xl border border-white/10 bg-white/5 p-4"><span className="text-xs font-black tracking-[.14em] text-sky-300">{String(index + 1).padStart(2, "0")}</span><h2 className="mt-2 text-sm font-black leading-5 text-white">{item[0]}</h2><p className="mt-2 text-xs leading-5 text-slate-300">{item[1]}</p>{index < 4 && <ChevronRight className="absolute -right-5 top-1/2 z-10 hidden h-7 w-7 -translate-y-1/2 rounded-full border border-emerald-300/30 bg-[#0b1724] p-1 text-emerald-300 xl:block"/>}</div>)}</div></header>

    <div className="space-y-6"><SectionTitle number="2.1" title={lang === "bm" ? "Sistem Respirasi" : "Human Respiratory System"} subtitle={lang === "bm" ? "Laluan udara dari hidung ke peparu" : "Air pathway from the nose to the lungs"}/><Panel><Flow items={lang === "bm" ? ["Lubang hidung", "Rongga hidung", "Farinks", "Larinks", "Trakea", "Bronkus", "Bronkiol", "Alveolus"] : ["Nostrils", "Nasal cavity", "Pharynx", "Larynx", "Trachea", "Bronchus", "Bronchiole", "Alveolus"]}/><p className="mt-4 text-xs text-slate-400">{lang === "bm" ? "Epiglotis membuka trakea semasa bernafas dan menutupnya semasa bolus ditelan." : "The epiglottis opens the trachea during breathing and closes it when a bolus is swallowed."}</p></Panel><BreathingMechanism lang={lang}/><SectionTitle number="2.1B" title={lang === "bm" ? "Bandingkan udara sedutan dengan udara hembusan" : "Compare inhaled and exhaled air"}/><BreathingExperiments lang={lang}/><Check lang={lang} question={section(0)?.checks[0]?.question ?? ""} answer={section(0)?.checks[0]?.hint ?? ""}/></div>

    <div className="space-y-6"><SectionTitle number="2.2" title={section(1).title} subtitle={lang === "bm" ? "Gas bergerak secara resapan mengikut kecerunan kepekatan; hemoglobin mengangkut oksigen di antara kedua-dua tempat." : "Gases diffuse down concentration gradients; haemoglobin carries oxygen between the two locations."}/><GasExchange lang={lang}/><Check lang={lang} question={section(1)?.checks[0]?.question ?? ""} answer={section(1)?.checks[0]?.hint ?? ""}/></div>

    <div className="space-y-6"><SectionTitle number="2.3" title={lang === "bm" ? "Kenal pasti bahan, kerosakan dan penyakit" : "Health of Human Respiratory System"}/><PollutantExplorer lang={lang}/><DiseaseCards lang={lang}/><SectionTitle number="2.3A" title={lang === "bm" ? "Eksperimen 2.2 mendedahkan tar dan gas berasid" : "Experiment 2.2 reveals tar and acidic gases"}/><SmokingExperiment lang={lang}/><Check lang={lang} question={section(2)?.checks[0]?.question ?? ""} answer={section(2)?.checks[0]?.hint ?? ""}/></div>

    <div className="space-y-6"><SectionTitle number="2.4" title={section(3).title} subtitle={lang === "bm" ? "Permukaan yang lembap, nipis dan luas mempercepat pertukaran gas dalam semua organisma." : "A moist, thin and large surface speeds gaseous exchange in every organism."}/><AdaptationExplorer lang={lang}/><Check lang={lang} question={section(3)?.checks[0]?.question ?? ""} answer={section(3)?.checks[0]?.hint ?? ""}/></div>

    <div className="space-y-6"><SectionTitle number="2.5" title={lang === "bm" ? "Tumbuhan menukar gas mengikut cahaya, kepekatan dan air" : "Gaseous Exchange in Plants"}/><PlantGasExchange lang={lang}/><SectionTitle number="2.5A" title={lang === "bm" ? "Pencemaran mengganggu pertukaran gas dan fotosintesis" : "Pollution disrupts gaseous exchange and photosynthesis"}/><PlantPollution lang={lang}/><Check lang={lang} question={section(4)?.checks[0]?.question ?? ""} answer={section(4)?.checks[0]?.hint ?? ""}/></div>
