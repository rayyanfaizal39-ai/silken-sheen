...
describe("ScienceF3Chapter9VisualNotesBlock", () => {
  it("renders the complete Malay visual-learning path", () => {
    ...
    expect(html).toContain("Cuaca Angkasa");
    expect(html).not.toContain("Puluhan minit");
    expect(html).not.toContain("Dari denyutan Matahari ke sistem di Bumi");
    expect(html).not.toContain("Enam lapisan, satu aliran tenaga");
    expect(html).not.toContain("Tujuh wajah Matahari yang aktif");
    expect(html).not.toContain("Magnetosfera ialah perisai dinamik");
    expect(html).not.toContain("Lebih banyak tompok, lebih aktif cuaca angkasa");
  ...
  it("renders the same visual-learning path in English", () => {
    ...
    expect(html).not.toContain("Tens of minutes");
    expect(html).not.toContain("From the Sun&#x27;s pulse to systems on Earth");
    expect(html).not.toContain("Six layers, one energy flow");
    expect(html).not.toContain("Seven faces of an active Sun");
    expect(html).not.toContain("The magnetosphere is a dynamic shield");
    expect(html).not.toContain("More sunspots, more active space weather");
  });
});
