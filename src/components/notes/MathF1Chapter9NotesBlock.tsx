...
import { PolygonDiagonalDiagram } from "./blocks/PolygonDiagonalDiagram";
...
const MARK_READ_BTN: Record<MathLang, string> = {
  en: "📘 Mark Chapter 9 as Read",
  bm: "📘 Tandakan Bab 9 Selesai",
};
const MARKED_BTN: Record<MathLang, string> = { en: "Marked as read ✓", bm: "Selesai ditanda ✓" };

/**
 * Form 1 Mathematics, Chapter 9 (Basic Polygons) interactive notes. Follows
 * the same composition as MathF1Chapter1NotesBlock. 9.1 embeds a DataTable
 * reference table plus a parameterized PolygonDiagonalDiagram in the lesson
 * intro; 9.3 embeds an inline FormulaSheet grid of quadrilateral types
 * before the angle-rule FormulaCard.
 */
export function MathF1Chapter9NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF1C9Content; bm: MathF1C9Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f1-c9-${s.num}`, weight: 3 })),
    { id: "math-f1-c9-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f1-c9-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.refTable && (
              <div className="mt-4">
                <DataTable headers={sub.refTable.headers} rows={sub.refTable.rows} />
              </div>
            )}
            {sub.polygonDemo && (
              <PolygonDiagonalDiagram
                n={sub.polygonDemo.n}
                countLabel={sub.polygonDemo.countLabel}
                caption={sub.polygonDemo.caption}
              />
            )}
          </LessonIntro>

          {sub.quadTypesSheet && <FormulaSheet entries={sub.quadTypesSheet} />}
      ...
      <div data-notes-section-id="math-f1-c9-chapter-end" className="mt-14 space-y-10">
        <div>
