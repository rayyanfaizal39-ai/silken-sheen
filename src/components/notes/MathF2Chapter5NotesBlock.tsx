...
import {
  CirclePartsDiagram,
  ChordBisectionDiagram,
  CircleUnrollDiagram,
} from "./blocks/CircleDiagrams";

const MARK_READ_BTN: Record<MathLang, string> = {
  en: "📘 Mark Chapter 5 as Read",
  bm: "📘 Tandakan Bab 5 Selesai",
};
const MARKED_BTN: Record<MathLang, string> = { en: "Marked as read ✓", bm: "Selesai ditanda ✓" };

/** Form 2 Mathematics, Chapter 5 (Circles) interactive notes. */
export function MathF2Chapter5NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C5Content; bm: MathF2C5Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c5-${s.num}`, weight: 3 })),
    { id: "math-f2-c5-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c5-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "circleParts" && (
              <CirclePartsDiagram
                centreLabel={sub.visual.centreLabel}
                radiusLabel={sub.visual.radiusLabel}
                diameterLabel={sub.visual.diameterLabel}
                chordLabel={sub.visual.chordLabel}
                arcLabel={sub.visual.arcLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "chordBisect" && (
              <ChordBisectionDiagram
                equalHalvesLabel={sub.visual.equalHalvesLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "circleUnroll" && (
              <CircleUnrollDiagram caption={sub.visual.caption} />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f2-c5-chapter-end" className="mt-14 space-y-10">
        <div>
