...
import { ScaleCompareDiagram } from "./blocks/ScaleDrawingDiagrams";
...
/** Form 3 Mathematics, Chapter 4 (Scale Drawings) interactive notes. */
export function MathF3Chapter4NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF3C4Content; bm: MathF3C4Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s, i) => ({ id: `math-f3-c4-sub-${i}`, weight: 3 })),
    { id: "math-f3-c4-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub, i) => (
        <div key={i} data-notes-section-id={`math-f3-c4-sub-${i}`} className="mt-14 first:mt-0">
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "scaleCompare" && (
              <ScaleCompareDiagram
                smallerLabel={sub.visual.smallerLabel}
                actualLabel={sub.visual.actualLabel}
                biggerLabel={sub.visual.biggerLabel}
      ...
      <div data-notes-section-id="math-f3-c4-chapter-end" className="mt-14 space-y-10">
        <div>
