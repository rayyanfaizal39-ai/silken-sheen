...
import { DistanceGridDiagram, MidpointDiagram } from "./blocks/CoordinateDiagrams";
...
/**
 * Form 2 Mathematics, Chapter 7 (Coordinates) interactive notes. 7.3 is not
 * a new theory topic — it's a "solving problems" subtopic that combines
 * distance and midpoint, so it has no dedicated visual/formula of its own.
 */
export function MathF2Chapter7NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C7Content; bm: MathF2C7Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c7-${s.num}`, weight: 3 })),
    { id: "math-f2-c7-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c7-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "distanceGrid" && (
              <DistanceGridDiagram
                horizontalLabel={sub.visual.horizontalLabel}
                verticalLabel={sub.visual.verticalLabel}
                distanceLabel={sub.visual.distanceLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "midpoint" && (
              <MidpointDiagram
                pointALabel={sub.visual.pointALabel}
                pointBLabel={sub.visual.pointBLabel}
                midpointLabel={sub.visual.midpointLabel}
                caption={sub.visual.caption}
              />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f2-c7-chapter-end" className="mt-14 space-y-10">
        <div>
