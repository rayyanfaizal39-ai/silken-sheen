...
/** Form 2 Mathematics, Chapter 11 (Isometric Transformations) interactive notes. */
export function MathF2Chapter11NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C11Content; bm: MathF2C11Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c11-${s.num}`, weight: 3 })),
    { id: "math-f2-c11-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c11-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "transformCompare" && (
              <TransformCompare
                validLabel={sub.visual.validLabel}
                invalidLabel={sub.visual.invalidLabel}
              />
            )}
            {sub.visual?.kind === "vectorArrow" && (
              <VectorArrow
                sidewaysLabel={sub.visual.sidewaysLabel}
                upLabel={sub.visual.upLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "mirror" && (
              <MirrorDiagram
                axisLabel={sub.visual.axisLabel}
                objectLabel={sub.visual.objectLabel}
                imageLabel={sub.visual.imageLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "rotation" && (
              <RotationDiagram
                centreLabel={sub.visual.centreLabel}
                objectLabel={sub.visual.objectLabel}
                imageLabel={sub.visual.imageLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "rotationalSymmetry" && (
              <RotationalSymmetry
                startLabel={sub.visual.startLabel}
                after1Label={sub.visual.after1Label}
                after2Label={sub.visual.after2Label}
                caption={sub.visual.caption}
              />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f2-c11-chapter-end" className="mt-14 space-y-10">
        <div>
