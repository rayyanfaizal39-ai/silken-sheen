...
/**
 * Form 1 Mathematics, Chapter 5 (Algebraic Expressions) interactive notes.
 * Structural copy of MathF1Chapter1NotesBlock — no chapter-specific
 * components needed, every subtopic reuses the standard block library.
 */
export function MathF1Chapter5NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF1C5Content; bm: MathF1C5Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f1-c5-${s.num}`, weight: 3 })),
    { id: "math-f1-c5-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f1-c5-${sub.num}`}
      ...
      <div data-notes-section-id="math-f1-c5-chapter-end" className="mt-14 space-y-10">
        <div>
