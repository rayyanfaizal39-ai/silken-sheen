...
import { CreditFlowDiagram, InterestGrowthChart } from "./blocks/InterestDiagrams";
...
/** Form 3 Mathematics, Chapter 3 (Consumer Mathematics) interactive notes. */
export function MathF3Chapter3NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF3C3Content; bm: MathF3C3Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f3-c3-${s.num}`, weight: 3 })),
    { id: "math-f3-c3-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f3-c3-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "interestCompare" && (
              <InterestGrowthChart
                principal={sub.visual.principal}
                rate={sub.visual.rate}
                years={sub.visual.years}
                simpleLabel={sub.visual.simpleLabel}
                compoundLabel={sub.visual.compoundLabel}
                yearAbbrev={sub.visual.yearAbbrev}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "creditFlow" && (
              <CreditFlowDiagram
                youLabel={sub.visual.youLabel}
                bankLabel={sub.visual.bankLabel}
                sellerLabel={sub.visual.sellerLabel}
                repayLaterLabel={sub.visual.repayLaterLabel}
                paysNowLabel={sub.visual.paysNowLabel}
                caption={sub.visual.caption}
              />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f3-c3-chapter-end" className="mt-14 space-y-10">
        <div>
