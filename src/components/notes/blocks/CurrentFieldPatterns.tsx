...
export function CurrentFieldPatterns({
  ...
  return (
    <div className="rounded-2xl border border-primary/25 bg-gradient-to-br from-primary/10 to-accent/5 p-3.5">
      ...
      {active?.notice && active.notice.length > 0 && (
        <div className="mt-2 rounded-xl border border-border bg-secondary/30 px-3 py-2">
          ...
          <ul className="mt-1 flex flex-col gap-0.5">
            {active.notice.map((n) => (
              <li
                key={n}
                className="text-[12px] leading-relaxed text-foreground before:mr-1.5 before:text-primary before:content-['•']"
              >
                {n}
              </li>
            ))}
