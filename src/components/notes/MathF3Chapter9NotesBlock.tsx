...
import {
  StraightLineParallelDiagram,
  StraightLineIntersectionDiagram,
} from "./blocks/StraightLineDiagrams";
...
/** Form 3 Mathematics, Chapter 9 (Straight Lines) interactive notes. */
export function MathF3Chapter9NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF3C9Content; bm: MathF3C9Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f3-c9-${s.num}`, weight: 3 })),
    { id: "math-f3-c9-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f3-c9-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "parallelLines" && (
              <StraightLineParallelDiagram caption={sub.visual.caption} />
            )}
            {sub.visual?.kind === "lineIntersection" && (
              <StraightLineIntersectionDiagram
                label={sub.visual.label}
      ...
      <div data-notes-section-id="math-f3-c9-chapter-end" className="mt-14 space-y-10">
        <div>
