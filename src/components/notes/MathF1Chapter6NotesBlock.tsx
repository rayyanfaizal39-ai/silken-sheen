...
/**
 * Form 1 Mathematics, Chapter 6 (Linear Equations) interactive notes.
 * Structural copy of MathF1Chapter1NotesBlock. 6.3 renders a second StepsCard
 * (secondWorked) so substitution gets the same step-reveal treatment as
 * elimination, instead of being relegated to the guided-card slot.
 */
export function MathF1Chapter6NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF1C6Content; bm: MathF1C6Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f1-c6-${s.num}`, weight: 3 })),
    { id: "math-f1-c6-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f1-c6-${sub.num}`}
        ...
        >
          ...
          {sub.secondWorked && (
            <StepsCard
              lang={lang}
              question={sub.secondWorked.question}
              steps={sub.secondWorked.steps}
            />
          )}
      ...
      <div data-notes-section-id="math-f1-c6-chapter-end" className="mt-14 space-y-10">
        <div>
