...
import {
  RegularVsIrregularDiagram,
  TriangleFanDiagram,
  PolygonWalkAroundDiagram,
} from "./blocks/PolygonDiagrams";
...
/** Form 2 Mathematics, Chapter 4 (Polygon) interactive notes. */
export function MathF2Chapter4NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C4Content; bm: MathF2C4Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c4-${s.num}`, weight: 3 })),
    { id: "math-f2-c4-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c4-${sub.num}`}
          ...
          {sub.blocks.map((block, bi) => (
            <div key={bi}>
              <LessonIntro lang={lang} tag={block.ideaTag} paragraphs={block.ideaParagraphs}>
                {block.visual?.kind === "regularVsIrregular" && (
                  <RegularVsIrregularDiagram
                    regularLabel={block.visual.regularLabel}
                    irregularLabel={block.visual.irregularLabel}
                    caption={block.visual.caption}
                  />
                )}
                {block.visual?.kind === "triangleFan" && (
                  <TriangleFanDiagram n={block.visual.n} caption={block.visual.caption} />
                )}
                {block.visual?.kind === "walkAround" && (
                  <PolygonWalkAroundDiagram n={block.visual.n} caption={block.visual.caption} />
                )}
              </LessonIntro>
      ...
      <div data-notes-section-id="math-f2-c4-chapter-end" className="mt-14 space-y-10">
        <div>
