...
import { TriangleLabel } from "./blocks/TrigRatioDiagrams";
...
/** Form 3 Mathematics, Chapter 5 (Trigonometric Ratios) interactive notes. */
export function MathF3Chapter5NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF3C5Content; bm: MathF3C5Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s, i) => ({ id: `math-f3-c5-sub-${i}`, weight: 3 })),
    { id: "math-f3-c5-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub, i) => (
        <div key={i} data-notes-section-id={`math-f3-c5-sub-${i}`} className="mt-14 first:mt-0">
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "triangleLabel" && (
              <TriangleLabel
                anchor={sub.visual.anchor}
                thetaLabel={sub.visual.thetaLabel}
                oppositeLabel={sub.visual.oppositeLabel}
                adjacentLabel={sub.visual.adjacentLabel}
                hypotenuseLabel={sub.visual.hypotenuseLabel}
      ...
      <div data-notes-section-id="math-f3-c5-chapter-end" className="mt-14 space-y-10">
        <div>
