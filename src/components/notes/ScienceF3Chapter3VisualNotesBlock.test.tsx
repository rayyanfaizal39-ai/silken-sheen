...
import {
  chapter3Headings,
  chapter3Lessons,
} from "@/content/form3/science/chapter-3/approved-notes";

describe("Chapter 3 approved Notes coverage", () => {
  it.each(["bm", "en"] as const)(
    "renders all approved paragraphs and five official sections through the registered %s path",
    async (lang) => {
      const registered = scienceF3ChapterContent.find(
        (c) => c.chapterKey === "Chapter 3" && c.lang === (lang === "bm" ? "bm" : "dlp"),
      )!;
      const content = registered.sciF3InteractiveData!;
      const html = renderToStaticMarkup(
        createElement(ScienceF3Chapter3VisualNotesBlock, {
      ...
      expect(html.replace(/<[^>]*>/g, " ")).not.toMatch(
        /Master Coverage|NotebookLM|\[\d+(?:,\s*\d+)*\]|(?:Rajah|Jadual|Aktiviti|Activity|Experiment) 3\.\d|3\.[1-5]\.[0-9]|Pelembaban|Open · water|Terbuka · air/,
      );
      const stream = await renderToReadableStream(
        createElement(ScienceF3InteractiveNotesBlock, {
          content,
          lang,
          storageKey: "chapter3-test",
        }),
      );
      const routed = await new Response(stream).text();
