...
import { InequalityNumberLine } from "./blocks/InequalityNumberLine";
...
/**
 * Form 1 Mathematics, Chapter 7 (Linear Inequalities) interactive notes.
 * Follows the same composition as MathF1Chapter1NotesBlock. 7.1's worked
 * example renders two InequalityNumberLine diagrams instead of StepsCard,
 * since the question asks to represent both inequalities visually.
 */
export function MathF1Chapter7NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF1C7Content; bm: MathF1C7Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f1-c7-${s.num}`, weight: 3 })),
    { id: "math-f1-c7-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f1-c7-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.ineqNumline && (
              <InequalityNumberLine
                boundary={sub.ineqNumline.boundary}
                direction={sub.ineqNumline.direction}
                inclusive={sub.ineqNumline.inclusive}
                min={sub.ineqNumline.min}
                max={sub.ineqNumline.max}
                label={sub.ineqNumline.label}
              />
            )}
          </LessonIntro>
          ...
          {sub.worked.ineqNumlines ? (
            <div className="mt-4 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-5 sm:p-6">
              ...
              {sub.worked.ineqNumlines.map((nl, i) => (
                <InequalityNumberLine
                  key={i}
                  boundary={nl.boundary}
                  direction={nl.direction}
                  inclusive={nl.inclusive}
                  min={nl.min}
                  max={nl.max}
                  label={nl.label}
                />
      ...
      <div data-notes-section-id="math-f1-c7-chapter-end" className="mt-14 space-y-10">
        <div>
