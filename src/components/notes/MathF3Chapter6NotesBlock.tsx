...
import {
  ...
  TangentRadius,
  TwoTangentsDiagram,
} from "./blocks/CircleTangentDiagrams";
...
/** Form 3 Mathematics, Chapter 6 (Angles and Tangents of Circles) interactive notes. */
export function MathF3Chapter6NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF3C6Content; bm: MathF3C6Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f3-c6-${s.num}`, weight: 3 })),
    { id: "math-f3-c6-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f3-c6-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "centralAngle" && (
              <CentralAngleDiagram caption={sub.visual.caption} />
            )}
            {sub.visual?.kind === "cyclicQuad" && (
              <CyclicQuadDiagram caption={sub.visual.caption} />
            )}
            {sub.visual?.kind === "tangentRadius" && (
              <TangentRadius
                radiusLabel={sub.visual.radiusLabel}
                tangentLabel={sub.visual.tangentLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "twoTangents" && (
              <TwoTangentsDiagram caption={sub.visual.caption} />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f3-c6-chapter-end" className="mt-14 space-y-10">
        <div>
