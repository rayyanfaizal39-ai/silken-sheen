...
import { DiceSampleGrid, ProbabilityNumberLine } from "./blocks/ProbabilityDiagrams";
...
/** Form 2 Mathematics, Chapter 13 (Simple Probability) interactive notes. */
export function MathF2Chapter13NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C13Content; bm: MathF2C13Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c13-${s.num}`, weight: 3 })),
    { id: "math-f2-c13-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c13-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "probLine" && (
              <ProbabilityNumberLine
                wontHappenLabel={sub.visual.wontHappenLabel}
                fiftyFiftyLabel={sub.visual.fiftyFiftyLabel}
                willHappenLabel={sub.visual.willHappenLabel}
              />
            )}
            {sub.visual?.kind === "diceGrid" && (
              <DiceSampleGrid targetSum={sub.visual.targetSum} caption={sub.visual.caption} />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f2-c13-chapter-end" className="mt-14 space-y-10">
        <div>
