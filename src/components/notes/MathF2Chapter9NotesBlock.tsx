...
import { UniformCompareDiagram, SpeedTimeRampDiagram } from "./blocks/SpeedDiagrams";
...
/** Form 2 Mathematics, Chapter 9 (Speed and Acceleration) interactive notes. */
export function MathF2Chapter9NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C9Content; bm: MathF2C9Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c9-${s.num}`, weight: 3 })),
    { id: "math-f2-c9-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c9-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "uniformCompare" && (
              <UniformCompareDiagram
                goodPoints={sub.visual.goodPoints}
                badPoints={sub.visual.badPoints}
                goodLabel={sub.visual.goodLabel}
                badLabel={sub.visual.badLabel}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "speedTimeRamp" && (
              <SpeedTimeRampDiagram
                acceleratingLabel={sub.visual.acceleratingLabel}
                deceleratingLabel={sub.visual.deceleratingLabel}
                caption={sub.visual.caption}
              />
            )}
          </LessonIntro>
          ...
          {sub.blocks.map((block, bi) => (
            <div key={bi}>
              <FormulaCard eyebrow={block.formula.eyebrow} formula={block.formula.formula} />
              {block.worked && (
                <StepsCard
                  lang={lang}
      ...
      <div data-notes-section-id="math-f2-c9-chapter-end" className="mt-14 space-y-10">
        <div>
