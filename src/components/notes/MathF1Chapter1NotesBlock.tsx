...
/**
 * Form 1 Mathematics, Chapter 1 (Rational Numbers) interactive notes.
 * Master template for every future Mathematics chapter — see
 * src/components/notes/blocks/ for the reusable pieces this composes.
 */
export function MathF1Chapter1NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF1C1Content; bm: MathF1C1Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f1-c1-${s.num}`, weight: 3 })),
    { id: "math-f1-c1-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f1-c1-${sub.num}`}
      ...
      <div data-notes-section-id="math-f1-c1-chapter-end" className="mt-14 space-y-10">
        <div>
