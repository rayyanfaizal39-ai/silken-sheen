...
import {
  LocusTraceDiagram,
  ThreeLociTypesDiagram,
  LociIntersectionDiagram,
} from "./blocks/LociDiagrams";
...
/** Form 3 Mathematics, Chapter 8 (Loci in Two Dimensions) interactive notes. */
export function MathF3Chapter8NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF3C8Content; bm: MathF3C8Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f3-c8-${s.num}`, weight: 3 })),
    { id: "math-f3-c8-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f3-c8-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual?.kind === "locusTrace" && (
              <LocusTraceDiagram
                pointLabel={sub.visual.pointLabel}
                resultLabel={sub.visual.resultLabel}
              />
            )}
            {sub.visual?.kind === "threeLociTypes" && (
              <ThreeLociTypesDiagram
                label1={sub.visual.label1}
                label2={sub.visual.label2}
                label3={sub.visual.label3}
                caption={sub.visual.caption}
              />
            )}
            {sub.visual?.kind === "lociIntersection" && (
              <LociIntersectionDiagram caption={sub.visual.caption} />
            )}
          </LessonIntro>
      ...
      <div data-notes-section-id="math-f3-c8-chapter-end" className="mt-14 space-y-10">
        <div>
