...
import { RiseRunTriangle } from "./blocks/GradientDiagrams";
...
/** Form 2 Mathematics, Chapter 10 (Gradient of a Straight Line) interactive notes. */
export function MathF2Chapter10NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C10Content; bm: MathF2C10Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c10-${s.num}`, weight: 3 })),
    { id: "math-f2-c10-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c10-${sub.num}`}
        ...
        >
          ...
          <LessonIntro lang={lang} paragraphs={sub.ideaParagraphs}>
            {sub.visual && (
              <RiseRunTriangle
                runLabel={sub.visual.runLabel}
                riseLabel={sub.visual.riseLabel}
              ...
              {block.worked && (
                <StepsCard
                  ...
                  diagram={
                    block.worked.visual && (
                      <RiseRunTriangle
                        runLabel={block.worked.visual.runLabel}
                        riseLabel={block.worked.visual.riseLabel}
          ...
          <GuidedCard
            ...
            diagram={
              sub.guided.visual && (
                <RiseRunTriangle
                  runLabel={sub.guided.visual.runLabel}
                  riseLabel={sub.guided.visual.riseLabel}
                  caption={sub.guided.visual.caption}
                />
              )
            }
          />
      ...
      <div data-notes-section-id="math-f2-c10-chapter-end" className="mt-14 space-y-10">
        <div>
