...
import type { GeoF3InteractiveContent, GeoMiniQuizItem } from "@/content/form3/geography/interactive-types";
...
function MiniQuiz({ item }: { item: GeoMiniQuizItem }) {
  ...
  function respond(value: number | boolean) {
    ...
    setAnswer(value);
    const isCorrect = item.type === "true-false" ? value === item.answer : value === item.answerIndex;
  ...
  return (
    <section id={id} className="mt-8 flex min-w-0 flex-col gap-9 animate-fade-up">
        ...
        <div key={section.number} className="flex min-w-0 flex-col gap-5">
          ...
          {section.zoneExplorers?.map((explorer, ei) => (
            <div key={`${section.number}-explorer-${ei}`}>
              <h3 className="font-display mb-2 text-base font-bold text-foreground">{explorer.heading}</h3>
              <p className="mb-2 text-[13px] leading-relaxed text-muted-foreground">{explorer.block.instruction}</p>
          ...
          {section.pieCalculator && (
            <div>
              <h3 className="font-display mb-2 text-base font-bold text-foreground">{section.pieCalculator.title}</h3>
              <p className="text-[13px] leading-relaxed text-muted-foreground">{section.pieCalculator.instruction}</p>
              <PieChartCalculator valueField={section.pieCalculator.valueField} totalField={section.pieCalculator.totalField} />
            </div>
          )}
          <div>
            <h3 className="font-display mb-2 text-base font-bold text-foreground">Semak diri — {section.number}</h3>
            <Accordion type="single" collapsible>
      ...
      <div>
        ...
        <SelfReflectionChecklist items={content.reflectionItems} storageKey={storageKey ? `${storageKey}:geo-f3-c${content.chapter}-reflection` : undefined} onAllComplete={() => awardOnce("reflection", 10)} />
      </div>
      <div>
        <h2 className="font-display mb-3 text-xl font-bold text-foreground">Kuiz Pantas</h2>
        <div className="flex flex-col gap-3">{content.miniQuiz.map((item, i) => <MiniQuiz key={i} item={item} />)}</div>
      </div>
      {onMarkRead && (
        <div className="flex justify-center">
          <button type="button" disabled={isRead} onClick={onMarkRead} className={`inline-flex min-h-11 items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold transition-transform duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${isRead ? "bg-emerald-500/20 text-emerald-200" : "bg-gradient-to-r from-primary to-accent text-white hover:scale-105 active:scale-[0.98]"}`}>
            <CheckCircle2 className="h-4 w-4" /> {isRead ? "Selesai ditanda" : `Tandakan Bab ${content.chapter} Selesai`}
          </button>
        </div>
      )}
    </section>
