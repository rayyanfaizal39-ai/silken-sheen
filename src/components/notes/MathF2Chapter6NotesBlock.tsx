...
import { Shape3DIcons, CubeNetDiagram, ConeCylinderVolumeDiagram } from "./blocks/Shape3DDiagrams";

const MARK_READ_BTN: Record<MathLang, string> = {
  en: "📘 Mark Chapter 6 as Read",
  bm: "📘 Tandakan Bab 6 Selesai",
};
 ...
 * interactive notes. Each subtopic renders as one or more intro+formula
 * blocks, followed by a single shared worked example — matching the source
 * mockup structure (e.g. 6.4 has two formula blocks — Prisms/Cylinders and
 * Pyramids/Cones — but only one worked example, placed after both).
 */
export function MathF2Chapter6NotesBlock({
  id,
  content,
  lang,
  isRead,
  ...
  id?: string;
  content: { en: MathF2C6Content; bm: MathF2C6Content };
  lang: MathLang;
  storageKey?: string;
  isRead?: boolean;
  onMarkRead?: () => void;
  ...
  const manifest = [
    ...t.subtopics.map((s) => ({ id: `math-f2-c6-${s.num}`, weight: 3 })),
    { id: "math-f2-c6-chapter-end", weight: 2 },
  ];
  ...
  return (
    ...
    >
      {t.subtopics.map((sub) => (
        <div
          key={sub.num}
          data-notes-section-id={`math-f2-c6-${sub.num}`}
          ...
          {sub.blocks.map((block, bi) => (
            <div key={bi}>
              <LessonIntro lang={lang} tag={block.ideaTag} paragraphs={block.ideaParagraphs}>
                {block.visual?.kind === "shapeIcons" && (
                  <Shape3DIcons labels={block.visual.labels} caption={block.visual.caption} />
                )}
                {block.visual?.kind === "cubeNet" && (
                  <CubeNetDiagram caption={block.visual.caption} />
                )}
                {block.visual?.kind === "coneCylinderVolume" && (
                  <ConeCylinderVolumeDiagram caption={block.visual.caption} />
                )}
              </LessonIntro>
      ...
      <div data-notes-section-id="math-f2-c6-chapter-end" className="mt-14 space-y-10">
        <div>
