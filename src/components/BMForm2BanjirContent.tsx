...
import { BANJIR_DI_MATA_EMAK as note } from "@/data/bm-form2-banjir-di-mata-emak";

const palette = ["#FB7185", "#FBBF24", "#34D399", "#60A5FA", "#C084FC"];
function Section({
  title,
  eyebrow,
  color = "#C084FC",
...
export function BMForm2BanjirContent() {
  return (
    <div>
      <Section title="Ketabahan diuji, kasih disedari" eyebrow="Fokus Pembelajaran" color="#60A5FA">
      ...
      <Section title="Gaya Bahasa" eyebrow="Bahasa Cerita" color="#FBBF24">
        <div className="grid gap-3 sm:grid-cols-2">
          {note.language.map((x) => (
            <Card key={x.title}>
              <h3 className="font-bold text-white">{x.title}</h3>
      ...
      <Section title="Nilai Murni & Pengajaran" eyebrow="Bina Peribadi" color="#FB7185">
        <div className="grid gap-3 sm:grid-cols-2">
          {note.values.map((x, i) => (
            <Card key={x.title}>
              <div className="mb-3 flex items-center gap-2">
                <Heart className="h-4 w-4" style={{ color: palette[i] }} />
                <h3 className="font-bold text-white">{x.title}</h3>
              </div>
