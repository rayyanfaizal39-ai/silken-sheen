...
const ORIGINALS: Record<string, Record<string, string>> = {
  BM: {
    ...
    "content.sections[3].accordions[2].body":
    ...
    "content.sections[10].accordions[0].body":
    ...
    "content.sections[10].accordions[1].body":
  ...
  it("marks the equivalent concept in both languages, in the same fields", () => {
    const fieldsWithMarkers = (content: ScienceF2InteractiveContent) =>
      allStrings(content)
        ...
        .map(([path]) => path)
        .sort();
    expect(fieldsWithMarkers(scienceF2C8InteractiveBM)).toEqual(
      fieldsWithMarkers(scienceF2C8InteractiveDLP),
    );
  });
